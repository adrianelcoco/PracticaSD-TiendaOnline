...
public class OrderController {
    ...
    public String orderPlaced(Model model, @RequestParam("productsId") List<String> productsId,
        ...
        products=cartService.getProductsInCart();
        List<Product> orderProducts=new ArrayList<>();
        for(int i=0;i< products.size();i++){ //Add the products from the cart to a new products arrayList
            Product product= products.get(i);
            orderProducts.add(product);
        }
        Order order = new Order(orderProducts,id,totalPrice,name,street,payMethod);
